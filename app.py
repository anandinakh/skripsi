...
# Konfigurasi koneksi SQLAlchemy
DATABASE_TYPE = 'mysql'
DBAPI = 'pymysql'
HOST = 'localhost'
USER = 'root'
...
def hitung():
    ...
    cur.execute('''
        INSERT INTO history (name, gender, age, height, weight, timestamp, prediction)
...
def print_history():
    try:
        ...
        cur = mysql.connection.cursor()
        cur.execute("SELECT name, gender, age, height, weight, timestamp, prediction FROM history ORDER BY timestamp DESC LIMIT 1")
...
def history():
    ...
    history_list = [History(row['id'], row['name'], row['gender'], row['age'], row['height'], row['weight'], row['timestamp'], row['prediction']) for row in history_data]
